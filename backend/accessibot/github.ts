...
// Indicates whether GitHub integration is in demo mode (no GitHub token configured).
...
export const createPullRequest = api<CreatePullRequestRequest, CreatePullRequestResponse>(
  ...
  async (req) => {
    ...
    // DEMO MODE: If no token configured, simulate PR creation
    if (!token || token.trim() === "") {
      const branchName = `accessibility-fixes-demo-${Date.now()}`;
      const demoUrl = "https://github.com/demo/repo/pull/1";
      console.log("GitHub demo mode active - simulating PR creation");
      return {
        pullRequestUrl: demoUrl,
        branchName,
        success: true,
        url: demoUrl,
        status: "Mock PR created (demo mode)",
      };
    }
    ...
    try {
      ...
      const repoResponse = await fetch(
        `https://api.github.com/repos/${req.repoOwner}/${req.repoName}`,
        {
          headers: {
      ...
      if (!repoResponse.ok) {
        ...
        if (repoResponse.status === 401) {
          throw APIError.unauthenticated(
            "Invalid GitHub token. Please check your token in the Infrastructure tab."
          );
        } else if (repoResponse.status === 404) {
          throw APIError.notFound(
            `Repository ${req.repoOwner}/${req.repoName} not found or you don't have access to it.`
          );
      ...
      const branchResponse = await fetch(
        `https://api.github.com/repos/${req.repoOwner}/${req.repoName}/git/refs/heads/${defaultBranch}`,
        {
          headers: {
            Authorization: `token ${token}`,
            Accept: "application/vnd.github.v3+json",
      ...
      if (!branchResponse.ok) {
        ...
        throw APIError.internal(
          `Failed to get branch info: ${branchResponse.status} ${branchResponse.statusText}`
        );
      }
      ...
      const createBranchResponse = await fetch(
        `https://api.github.com/repos/${req.repoOwner}/${req.repoName}/git/refs`,
        {
          method: "POST",
          headers: {
            Authorization: `token ${token}`,
      ...
      for (const fix of req.fixes) {
        ...
        const fileResponse = await fetch(
          `https://api.github.com/repos/${req.repoOwner}/${req.repoName}/contents/${fix.fileName}?ref=${branchName}`,
          {
            headers: {
              Authorization: `token ${token}`,
              Accept: "application/vnd.github.v3+json",
        ...
        const updateFileResponse = await fetch(
          `https://api.github.com/repos/${req.repoOwner}/${req.repoName}/contents/${fix.fileName}`,
          {
            method: "PUT",
            headers: {
              Authorization: `token ${token}`,
        ...
        if (!updateFileResponse.ok) {
          ...
          throw APIError.internal(
            `Failed to update file ${fix.fileName}: ${updateFileResponse.status} ${updateFileResponse.statusText}`
          );
        }
      }
      ...
      const description =
        req.description ||
        `This pull request fixes various accessibility issues found by AccessiBot.\n\nFixed issues:\n${req.fixes
          .flatMap((f) => f.issues)
          .map((issue) => `- ${issue}`)
          .join("\n")}`;

      const prResponse = await fetch(
        `https://api.github.com/repos/${req.repoOwner}/${req.repoName}/pulls`,
        {
          method: "POST",
          headers: {
            Authorization: `token ${token}`,
      ...
      if (!prResponse.ok) {
        ...
        throw APIError.internal(
          `Failed to create pull request: ${prResponse.status} ${prResponse.statusText}`
        );
      }
...
export const listRepositories = api<void, ListRepositoriesResponse>(
  ...
  async () => {
    ...
    // DEMO MODE: If no token configured, return mock repositories
    if (!token || token.trim() === "") {
      console.log("GitHub demo mode active - returning mock repositories list");
      return {
        repositories: [
          {
            id: 1,
            name: "accessibot-demo",
            full_name: "demo/accessibot-demo",
            description: "Demo repository for AccessiBot",
            private: false,
            html_url: "https://github.com/demo/accessibot-demo",
          },
          {
            id: 2,
            name: "website",
            ...
            private: false,
            html_url: "https://github.com/demo/website",
          },
          {
            id: 3,
            name: "internal-tools",
            full_name: "demo/internal-tools",
            description: "Internal tools (demo)",
            private: true,
            html_url: "https://github.com/demo/internal-tools",
          },
        ],
      };
    }
